...
with open('train_bl.csv', 'w', encoding='utf8') as train_:
    for row in tqdm(train.itertuples()):
        train_.write(ekhp_processor.preprocess(str(row[1])) + '\n')
        train_.write(ekhp_processor.preprocess(str(row[2])) + '\n')
        train_.write(ekhp_processor.preprocess(str(row[3])) + '\n')

with open('dev_bl.csv', 'w', encoding='utf8') as dev_:
    for row in tqdm(dev.itertuples()):
        dev_.write(ekhp_processor.preprocess(str(row[1])) + '\n')
        dev_.write(ekhp_processor.preprocess(str(row[2])) + '\n')
        dev_.write(ekhp_processor.preprocess(str(row[3])) + '\n')

with open('test_bl.csv', 'w', encoding='utf8') as test_:
    for row in tqdm(test.itertuples()):
        test_.write(ekhp_processor.preprocess(str(row[1])) + '\n')
        test_.write(ekhp_processor.preprocess(str(row[2])) + '\n')
